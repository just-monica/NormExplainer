...
class Mutagenicity(InMemoryDataset):
    ...
    def process(self):
        ...
        for i in range(original_labels.shape[0]):
            ...
            edge_attr = torch.ones((edge_index.size(1), 5)).float() 
            #dummy edge attributes although molecular graph should have different types of edges
            # why 5? just the same as other datasets
            ...
            data = Data(x=x, y=y.squeeze().long(), edge_index=edge_index, edge_attr=edge_attr, node_label=node_label.to(torch.long), edge_mask=edge_label.to(torch.long), node_type=torch.tensor(node_type_lists[i]), idx=mol_idx)
...
class MutagenicityNoFeature(InMemoryDataset):
    #another version of Mutagenicity dataset, without node features
    def __init__(self, root, name):
        self.name = name.lower()
        super().__init__(root=root)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self):
        ...
        max_num_nodes = 0
        mol_idx = 0
        for i in range(original_labels.shape[0]):#original_labels.shape[0] 就是图的数量
            num_nodes = len(node_type_lists[i])
            edge_index = torch.tensor(edge_lists[i], dtype=torch.long).T
            edge_attr = torch.ones((edge_index.size(1), 5)).float()
            #dummy edge attributes although molecular graph should have different types of edges
            ...
            y = y.squeeze()
            x = torch.tensor(original_features[i][:num_nodes]).float()
            x = torch.ones_like(x).float() #no node features
            assert original_features[i][num_nodes:].sum() == 0
            edge_label = torch.tensor(edge_label_lists[i]).float()
            if y.item() != 0:
                edge_label = torch.zeros_like(edge_label).float()
...
if __name__ == '__main__':
    '''
    dataset = Mutagenicity(root = './Datasets/DatasetsFromDIG',name='mutagenicity')
    print(len(dataset))
    ...
    '''
    dataset = MutagenicityNoFeature(root = './Datasets/DatasetsFromDIG',name='mutagenicity_no_feat')
    print(len(dataset))
    print(dataset.x.size())
    print(dataset.y.size())
    print(dataset.y.dtype)
    ...
    print(dataset.node_label.sum())
'''
from Datasets.DatasetsFromDIG.Mutagenicity import Mutagenicity
dataset = Mutagenicity(root = './Datasets/DatasetsFromDIG',name='mutagenicity')
'''