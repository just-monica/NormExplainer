...
class AlkaneCarbonyl(InMemoryDataset):
    ...
    def process(self):
        data = np.load(self.raw_dir + "/alkane_carbonyl.npz", allow_pickle=True)
        ...
        for i in range(len(X)):
            ...
            # y = X[i]['globals'][0]
            y = torch.tensor(ylist[i].astype(np.int32), dtype=torch.long)
            ...
            data_i = Data(
                ...
                edge_mask=gt_edge_mask.int(),
                idx=mol_idx,
                node_label=torch.max(node_imp, dim=1)[0].int(),
            )
...
if __name__ == '__main__':
    dataset = AlkaneCarbonyl(root = './Datasets/DatasetsFromPyG',name='alkanecarbonyl')
